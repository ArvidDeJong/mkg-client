...
- Caller values never reach MKG raw. `buildFilter()` sends an int, a strict numeric string (`-?\d+(\.\d+)?`) or a plain key of letters and digits with at least one digit (`VK2606096`) unquoted, exactly as 1.2 did, because nobody can test a changed wire format against every customer's MKG; none of those can extend the expression. Every other value goes through the quoted text filter. A bare word without a digit is quoted too: unquoted it could name a field, and `vorh_num = debi_num` compares two fields. Never widen the unquoted pattern with a space, an underscore, a hyphen or a dot. `quoteFilterText()` escapes the backslash BEFORE the quote (the other order lets a trailing backslash eat the closing quote) and refuses control characters. Every caller supplied URL part goes through `encodePathSegment()`, which refuses an empty part, `.` and `..`; `allowCompositeKey: true` keeps the `+` of a whole composite key (`1+10001`) so the URL stays what it was. A new finder uses these helpers; never concatenate an argument into a filter or a path. The `filter` argument of the `list…()` methods is MKG syntax by design and is passed through.
...
- Field metadata comes from the CSV files in `resources/csv/` (one per MKG document, semicolon separated, exported from MKG), loaded by [FieldMetaNormalizer](src/Support/FieldMetaNormalizer.php). It drives type normalisation and filters the built-in default `FieldList` of `ArticleService`, `DebtorsService`, `ContactpersonService` and `UserService`: a default field missing from the CSV is dropped silently, while a `fieldList` the caller passes is sent unfiltered. Only `integer`, `decimal`, `percentage`, `bedrag`, `logical`, `datum` and the text types are normalised; `aantal` and the rest pass through, and `debi_num` and `vorh_num` are `character`, so strings. The docs say so; keep them in step when a type is added. A host app can override a file under `storage/mkg/`. MKG rows arrive as `response.ResultData[0].{document}`.
