...
Use this skill when code reads from MKG in an application that has `darvis/mkg-client` installed, when a call ends in a `401`, a `403` or a `RuntimeException` about missing configuration, when rows or fields are missing without an error, when a sync stalls, or when you write tests around code that talks to MKG.
...
5. The JSON body is decoded to an array. The `find…Rows…` and `extract…Rows()` methods take the rows from `response.ResultData[0].{document}` and normalise them with the CSV metadata; a single row that MKG returns as an object is wrapped in a list.
...
| A `3xx`, or a `200` whose body is not JSON | `[]`, and `[]` from the row methods. Redirects are never followed and nothing throws | debug `MKG request completed.` only with `log_requests` on |
...
// Digits: debtor number and relation number, merged on RowKey (two requests).
...
- **An empty result is not proof that nothing exists.** A redirect or a non-JSON `200` also comes back as `[]`, and rows beyond 100 (or 1000) are cut off silently.
...
- **Filter values are not escaped or quoted by the number lookups.** `findHeaderByOrderNumber()`, `findByDebtorNumber()` and the like place the value into `Filter` as it is. Validate user input (digits, or your order number format) before passing it on. The same goes for the `get…ByPrimaryKey()` methods, which put their arguments into the URL path.
- **The cookie is written through the `Storage` facade, on the default filesystem disk.** With the default `cookie_storage_path` that is `mkg/cookie.txt` under the root of that disk. It is a live ERP session: make sure the default disk is not public. When the default disk is remote (`s3`), every new service instance reads the cookie from there. Services are not singletons, but they share the session through this file.
