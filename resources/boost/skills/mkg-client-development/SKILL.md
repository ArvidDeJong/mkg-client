...
- **A service class of your own is not bound by the package.** The first constructor argument is `?Client $client = null`, and the Laravel 11 container fills an optional class argument with a new object instead of null. The package binds its seven services in its provider, so they always build the configured client (up to 1.3.1 they did not, on Laravel 11). A class you wrote by extending one of them is resolved without that binding: `verify_ssl`, the timeouts, `log_requests` and `slow_request_seconds` then do nothing and redirects are followed. Bind it in a service provider: `$this->app->bind(InvoicesService::class, fn ($app) => new InvoicesService(logger: $app->make(LoggerInterface::class)))`.
