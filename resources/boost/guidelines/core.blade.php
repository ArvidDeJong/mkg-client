...
- Services are not singletons; each `app()` call builds a new one and they share the session through the cookie file. On Laravel 11 the container passes its own default Guzzle client into a resolved service, so `verify_ssl`, the timeouts, `log_requests` and the redirect setting do not apply; bind the services with `new DebtorsService(logger: $app->make(\Psr\Log\LoggerInterface::class))` there. Laravel 12 and 13 are not affected.
