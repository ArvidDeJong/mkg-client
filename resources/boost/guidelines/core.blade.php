...
- MKG caps a result set at 1000 rows per call and returns 100 when `NumRows` is omitted, without any error when more exist. Page with `skipRows` and `numRows`, and pass `sort` so the order is stable across pages.
...
- Filters use MKG's own syntax in the `filter` argument (`debi_num = 10001`, `vorh_num = VK2606096`); the services offer `buildFilter()` helpers internally, and field names stay exactly as MKG spells them (`debi_num`, `vorh_dat_order`).
...
- In plain PHP, construct a service with an `ArrayConfigProvider` (keys `mkg.host`, `mkg.customer`, `mkg.username`, `mkg.password`) and optionally a PSR-3 logger; in Laravel the provider, the `LaravelCookieStore` (cookie under `storage/mkg/cookie.txt`) and the logger are wired for you.
